...
| `chromeDevtoolsRelay` / `chrome_devtools_relay` | OpenClaw Chrome extension relay policy for eligible `chrome-devtools-mcp --autoConnect` stdio definitions: `prefer` (default), `require`, or `off`. `MCPORTER_CHROME_DEVTOOLS_RELAY_POLICY` overrides config. See the [daemon guide](daemon.md#chrome-devtools-through-the-openclaw-extension-relay). |
